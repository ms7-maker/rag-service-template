...
class RAGPipeline:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            data_file: путь к файлу с документами (если нет data/*_clean.txt)
        ...
        # Загрузка документов, если индекс пустой
        if self.vector_store.count() == 0:
            clean_docs = load_documents_from_data()
            if clean_docs:
                names = ", ".join(name for name, _ in clean_docs)
                print(f"Загрузка документов из data/ ({names})...")
                ...
                print(
                    "Предупреждение: нет data/*_clean.txt и нет "
                    f"{data_file} — индекс пустой"
                )
